...
public class SessionUtil
{
	...
	public static boolean hasAuth(String auth){
		ResgainUser user = getLoginUser();
		if("admin".equals(user.getUsername())) //这里主要是方便使用给admin增加了所有权限,也可以去掉
			return true;
		return user==null?false:user.hasAuth(auth);
	}

	public static String getRootPath() {
		return ActionContext.getHttpServletRequest().getServletContext().getRealPath("/");
	}
}
